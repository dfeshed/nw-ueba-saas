...
moduleForComponent('rsa-alerts-table/event', 'Integration | Component | rsa alerts table event', {
  integration: true,
...
test('it renders an enrichment if given an object with isEnrichment, an event otherwise', function(assert) {
  ...
  this.render(hbs`{{rsa-alerts-table/event item=item index=0}}`);

  return wait()
    .then(() => {
      assert.equal(this.$('.rsa-alerts-table-event').length, 1, 'Expected to find root DOM node.');
