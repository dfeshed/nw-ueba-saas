...
let setState;
const selectors = {
  loadSpinner: '.rsa-aggregation-rule .rsa-loader',
  form: '.rsa-aggregation-rule .aggregation-rule-form',
  ...
  groupingOptions: '.aggregation-rule-grouping-options',
...
moduleForComponent('rsa-aggregation-rule', 'Integration | Component | Respond Aggregation Rule', {
  integration: true,
...
const spyRuleUpdate = ({ state, test, assert, selector }) => {
  const actionSpy = sinon.spy(aggregationRuleCreators, 'updateRule');
  setState(state || { ...initialState });
  test.render(hbs`{{rsa-aggregation-rule ruleId='12345'}}`);
...
test('The component appears in the DOM', function(assert) {
  ...
  this.render(hbs`{{rsa-aggregation-rule ruleId='12345'}}`);
