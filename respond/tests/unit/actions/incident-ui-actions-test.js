...
module('Unit | Utility | Incident UI Actions - Reducers', function() {
  ...
  test('The TOGGLE_ENTITY_VISIBILITY action property adds the entity type to the visibileEntityTypes in state if it is not there', function(assert) {
    ...
    const endState = incidentReducer(initialState, {
      type: ACTION_TYPES.TOGGLE_ENTITY_VISIBILITY,
      payload: { entity: 'host' }
    });
  ...
  test('The TOGGLE_ENTITY_VISIBILITY action property removes the entity type to the visibileEntityTypes in state if it is already there', function(assert) {
    const initState = {
      ...initialState,
      visibleEntityTypes: ['host']
    };
    const expectedEndState = {
      ...initialState,
      visibleEntityTypes: []
