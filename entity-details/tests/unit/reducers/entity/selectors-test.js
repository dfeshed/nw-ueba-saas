...
import { entityId, entityType, entityDetails, entityDisplayName, entitySeverity, enityIcon, alertsForEntity, isFollowed, entityScore, entityFetchError } from 'entity-details/reducers/entity/selectors';
...
test('test entity state for error', function(assert) {
  assert.equal(entityFetchError(state), false);
  const newState = Immutable.from({
    entity: {
      entityFetchError: true
    }
  });
  assert.equal(entityFetchError(newState), true);
});
