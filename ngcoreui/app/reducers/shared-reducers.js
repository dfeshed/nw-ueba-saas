...
export default handleActions({
    ...
    return handle(state, action, {
      finish: (prevState) => {
        ...
        const operations = prevState.treePathContents.operations.map((op, index) => {
          if (index === prevState.treeSelectedOperationIndex) {
            return op.set('manual', man);
          } else {
            return op;
          }
        });
  ...
  [ACTION_TYPES.TREE_GET_OPERATION_PARAM_HELP]: (state, action) => {
    ...
    return handle(state, action, {
      finish: (prevState) => {
        ...
        // since we're breaking state into smaller pieces with combineReducers in app\reducers\index.js,
        // we'll need to wrap the passed 'state' to match the shape that the selector requires
        const fullPrevState = { shared: prevState };
        const operation = selectedOperation(fullPrevState);
  ...
  [ACTION_TYPES.TREE_UPDATE_PARAM]: (state, action) => {
    ...
    // since we're breaking state into smaller pieces with combineReducers in app\reducers\index.js,
    // we'll need to wrap the passed 'state' to match the shape that the selector requires
    const fullState = { shared: state };
    const operation = selectedOperation(fullState);
    let newOperation;
  ...
  [ACTION_TYPES.TREE_SELECT_OPERATION]: (state, action) => {
    ...
    const fullState = { shared: state };
    return state.merge({
      'treeSelectedOperationIndex': operationNames(fullState).indexOf(action.payload),
      'operationResponse': null,
      'treeOperationParams': {}
    });
  },
