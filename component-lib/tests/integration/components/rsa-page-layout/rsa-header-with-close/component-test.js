...
module('Integration | Component | rsa-header-with-close', function(hooks) {
  ...
  test('it renders', async function(assert) {
    ...
    assert.equal(find('.rsa-header .title').textContent, 'Title', 'Default title is displayed');

    this.set('title', 'Filters');
    await render(hbs`{{rsa-page-layout/rsa-header-with-close title=title}}`);
    assert.equal(find('.rsa-header .title').textContent, 'Filters', 'Custom title is displayed');
