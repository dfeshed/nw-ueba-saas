...
module('Integration | Component | rsa-area-series', function(hooks) {
  ...
  test('it renders', async function(assert) {
    await render(hbs `{{rsa-area-series}}`);
    assert.equal(findAll('.rsa-area-series').length, 1, 'Testing to see if the .rsa-area-series class exists');
  });
  ...
  test('generates path when supplied with required attributes', async function(assert) {
    ...
    await render(hbs `{{rsa-area-series data=data height=height xScale=xScale yScale=yScale}}`);
    setTimeout(function() {
      assert.equal(find('.rsa-area-series').getAttribute('d'), 'M0,150L200,75L400,0L400,150L200,150L0,150Z', 'Testing to see if the correct path was generated');
      done();
    }, 50);
  });
  ...
  test('generates path when supplied with multiple series of data', async function(assert) {
    ...
    await render(hbs `{{rsa-area-series data=data dataIndex=1 height=height xScale=xScale yScale=yScale}}`);
    setTimeout(function() {
      assert.equal(find('.rsa-area-series').getAttribute('d'), 'M0,150L200,75L400,0L400,150L200,150L0,150Z', 'Testing to see if the correct path was generated for the second data series');
      done();
    }, 50);
  });
  ...
  test('generates path when supplied with custom data accessor properties', async function(assert) {
    ...
    this.set('yProp', 'bar');
    await render(
      hbs `{{rsa-area-series data=data height=height xProp=xProp yProp=yProp xScale=xScale yScale=yScale}}`
    );
    setTimeout(function() {
      assert.equal(find('.rsa-area-series').getAttribute('d'), 'M0,150L200,75L400,0L400,150L200,150L0,150Z', 'Testing to see if the correct path was generated using custom x/y properties');
      done();
    }, 50);
  });
