...
public class AdeDataProcessingManager {
    ...
    public class ProcessModelFeatureBuckets implements Callable<Integer> {
        ...
        public Integer call() {
            ...
            SshResponse p4 = sshHelper.uebaHostExec().setUserDir(PRESIDIO_DIR).run(JAVA_CMD + PRESIDIO_ADE_APP_MODEL_FEATURE_BUCKETS + ".jar",
                    "run", "--schema " + schema.toUpperCase(), "--start_date " + start.toString(),
                    "--end_date " + end.toString(), "--fixed_duration_strategy " + getFixedDuration(timeFrame),
                    " > " + logPath);
    ...
    public class ProcessModeling implements Callable<Integer> {
        ...
        public Integer call() {
            ...
            SshResponse p4 = sshHelper.uebaHostExec().setUserDir(PRESIDIO_DIR).run(JAVA_CMD + PRESIDIO_ADE_APP_MODELING + ".jar",
