...
cd $RPMS_DIR
AWS_DIR_NAME=$(ls rsa-nw-presidio-core-*.el7.noarch.rpm  |  cut -d "-"  -f6  |  cut -d "."  -f1)
echo "$(sh /home/centos/scripts/update-s3-yum-repo/dist/update-s3-yum-repo-7/update-s3-yum-repo s3://team-ueba/RSA/UEBA-Repo/$AWS_DIR_NAME/)"

for f in $RPMS_DIR/*
do
  echo "going to upload  $f to S3"
  echo "$(sh /home/centos/scripts/update-s3-yum-repo/dist/update-s3-yum-repo-7/update-s3-yum-repo s3://team-ueba/RSA/UEBA-Repo/$AWS_DIR_NAME/ $f)"

done
}


if [ "$1" -ge 1 ] && [ "$1" -lt 6666 ]; then
	downoadRpmsFromJenkins
	uploadRpmsToS3ByUpdateS3YumRepo
elif [ "$1" == "only_upload" ]; then
	uploadRpmsToS3ByUpdateS3YumRepo
else
