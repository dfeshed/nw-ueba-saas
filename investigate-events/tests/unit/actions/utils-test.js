...
module('Unit | Helper | Actions Utils', function(hooks) {
  ...
  test('updateErrorMsgIfMaxMemory does return with text appended with match', function(assert) {
    ...
    assert.equal(result, 'max.query.memory warning. To avoid this error, try limiting the query by narrowing the time range, adding filters, removing complex filter operations, decreasing the number of columns in the column group, or ask an administrator to limit the number of events returned.');
  });
