...
module('Integration | Component | Query Pills', function(hooks) {
  ...
  test('Typing OpenParen followed by clicking the right arrow should delete the empty pill in the middle of pills along with empty parens and logical operator and moves the focus to the right', async function(assert) {
    new ReduxDataHelper(setState)
      .language()
      .canQueryGuided()
      .pillsDataPopulated()
    ...
    assert.equal(triggers.length, 3, 'correct number of triggers (3)');
    await click(triggers[1]);
    await typeIn(PILL_SELECTORS.metaSelectInput, '(');
    assert.equal(findAll(PILL_SELECTORS.logicalOperator).length, 2, 'Should have two logical operators');
    await triggerKeyEvent(PILL_SELECTORS.metaSelectInput, 'keydown', ARROW_RIGHT_KEY);
    // clicking backspace on the meta will move the focus to the left pill
    assert.equal(findAll(PILL_SELECTORS.logicalOperator).length, 1, 'Should have one logical operator');
  ...
  test('Typing OpenParen followed by logical operator should not be allowed whether one or multiple parens', async function(assert) {
    new ReduxDataHelper(setState)
      .language()
      .canQueryGuided()
      .pillsDataPopulated()
