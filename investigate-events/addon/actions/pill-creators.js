...
export const stashPills = (payload) => {

  return (dispatch, getState) => {
    const { actionType } = payload;
    switch (actionType) {
      case 'add': {
        const { investigate: { queryNode: { pillsData: originalPills } } } = getState();
        dispatch({
      ...
      case 'close': {
        const { investigate: { queryNode: { originalPills: pillsData } } } = getState();
        dispatch({
          type: ACTION_TYPES.STASH_PILLS_DATA,
          payload: {
            pillsData,
