...
test('it is a button', function(assert) {
  this.render(hbs `{{#rsa-form-button}}Label{{/rsa-form-button}}`);
  let buttonCount = this.$().find('button').length === 1;
...
test('it includes the proper classes', function(assert) {
  ...
  assert.ok(button.hasClass('rsa-form-button'));
});

test('it includes the proper attributes when isDisabled', function(assert) {
  this.render(hbs `{{#rsa-form-button isDisabled=true}}Label{{/rsa-form-button}}`);
  let buttonCount = this.$().find('button[disabled]').length;
  assert.equal(buttonCount, 1);
});
