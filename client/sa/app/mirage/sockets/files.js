...
function _fileForDestination(dest) {
  let file;
  if (dest) {
    for (let i = 0, len = _FILE_MAP.length; i < len; i++) {
      let match = dest.match(_FILE_MAP[i].regex);
      if (match) {
