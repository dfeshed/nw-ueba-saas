...
export default Ember.Service.extend({
   ...
   */
  connect(url, headers) {
    let promise = _connectionPromises[url];
    if (!promise) {
      ...
      promise = _connectionPromises[url] = new Ember.RSVP.Promise(function(resolve, reject) {
        ...
        let client = _stompClients[url] = Stomp.over(
   ...
   * @public
   */
  subscribe(url, destination, callback, headers) {
    let subs = _subscriptions[url] = _subscriptions[url] || {},
    ...
    if (!sub) {
      ...
      let me = this;
      return this.connect(url).then(function(client) {
        let subscription = client.subscribe(destination, _wrapCallback(callback), headers || {});
  ...
  stream(config, params, id) {
    ...
    let stream = params.stream = params.stream || {};
