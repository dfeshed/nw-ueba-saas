...
module('Integration | Component | alerts-tab/body/alerts-table/date-row/alert', function(hooks) {
  ...
  test('it should render alert row should show indicators inside alerts', async function(assert) {
    this.set('alerts', [[], alerts, [], []]);
    const done = assert.async();
    await render(hbs`{{alerts-tab/body/alerts-table/date-row/alert alerts=alerts}}`);
    await click('.alerts-tab_body_body-table_body_row_alerts_alert');
    return waitUntil(() => document.querySelectorAll('.rsa-data-table').length === 1, { timeout: 30000 }).then(async() => {
      assert.equal(findAll('.rsa-data-table-header').length, 1);
      await click('.alerts-tab_body_body-table_body_row_alerts_alert');
      return waitUntil(() => document.querySelectorAll('.rsa-data-table').length === 0, { timeout: 30000 }).then(async() => {
        assert.equal(findAll('.rsa-data-table-header').length, 0);
        done();
      });
    });
  });
