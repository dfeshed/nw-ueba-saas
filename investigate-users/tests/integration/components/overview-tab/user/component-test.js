...
import { render, find, findAll, click, settled, triggerEvent } from '@ember/test-helpers';
...
module('Integration | Component | overview-tab/user', function(hooks) {
  ...
  test('it should show proper count', async function(assert) {
    ...
    assert.equal(findAll('.user-overview-tab_users_row_alert-severity.Medium').length, 5);
    assert.equal(findAll('.user-overview-tab_users_row_alert-severity.Low').length, 5);
  ...
  test('it should open alert details on hover', async function(assert) {
    const redux = this.owner.lookup('service:redux');
    redux.dispatch(getUserOverview());
    await render(hbs `{{overview-tab/user}}`);
    await triggerEvent('.user-overview-tab_users_rows .rsa-content-tethered-panel-trigger', 'mouseover');
    return settled().then(() => {
      assert.equal(findAll('.user-overview-tab_users_row_alert_row').length, 4);
      assert.equal(findAll('.user-overview-tab_users_row_alert_row_severity').length, 4);
    });
