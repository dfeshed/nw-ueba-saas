...
module('Unit | Reducers | investigate', function(hooks) {
  ...
  test('the GET_AND_SAVE_ENABLE_STATUS gets the endpoint rar enable sttaus', function(assert) {
    const configData = {
      enabled: true,
      esh: 'esh-domain',
      servers: [
    ...
    const newEndState = reducer(previous, newAction);
    assert.deepEqual(newEndState.isEnabled, true);
  });
  ...
  test('The TEST_RAR_CONFIG action sets the loader to true on start', function(assert) {
    const configData = {
      enabled: true,
      esh: 'esh-domain',
      servers: [
    ...
    const newAction = makePackAction(LIFECYCLE.START, {
      ...
      payload: { rarConfig: { ...configData } }
    });

    const newEndState = reducer(previous, newAction);
    assert.deepEqual(newEndState.testConfigLoader, true);
  });
});