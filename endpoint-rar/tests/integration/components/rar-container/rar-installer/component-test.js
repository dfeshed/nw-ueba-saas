...
module('Integration | Component | rar-installer', function(hooks) {
  ...
  test('password validation content', async function(assert) {
    ...
    assert.equal(find('.input-error').textContent.trim(), 'Password must contain at least 3 characters.');
  });
  ...
  test('password validation empty', async function(assert) {
    ...
    await triggerKeyEvent('.password-input-js input', 'keyup', 65);
    assert.equal(find('.input-error').textContent.trim(), 'Enter the relay server installation password.');
  });
