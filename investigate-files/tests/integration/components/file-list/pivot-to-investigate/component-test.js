...
test('cancel should close the service modal', function(assert) {
  this.set('serviceList', [
    { 'id': 'e90bd2a2-a768-4cb9-a19d-37cd9f47fdcc', 'displayName': 'loki-concentrator', 'name': 'CONCENTRATOR' },
    { 'id': 'e90bd2a2-a768-4cb9-a19d-37cd9f47fdcc', 'displayName': 'loki-broker', 'name': 'BROKER' }
  ]);
  ...
  return wait().then(() => {
    assert.equal($('#modalDestination .service-modal').length, 1, 'Expected to render service modal');
    $('.is-standard .rsa-form-button').click();
    return wait().then(() => {
      assert.equal($('#modalDestination .service-modal').length, 0, 'Service modal is closed');
    });
  });
});
...
test('should open the event analysis page in new window', function(assert) {
  const actionSpy = sinon.spy(window, 'open');
  this.set('serviceList', [
    { 'id': 'e90bd2a2-a768-4cb9-a19d-37cd9f47fdcc', 'displayName': 'loki-concentrator', 'name': 'CONCENTRATOR' },
    { 'id': 'e90bd2a2-a768-4cb9-a19d-37cd9f47fdcc', 'displayName': 'loki-broker', 'name': 'BROKER' }
