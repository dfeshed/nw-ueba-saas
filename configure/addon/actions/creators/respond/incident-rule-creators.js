...
import {
  getAllCategories,
  getAllEnabledUsers
} from 'configure/actions/creators/respond/dictionary-creators';
import {
  getFields as getAggregationFields,
  getRuleConditions,
  getRuleConditionGroups,
  getRuleInfo,
  getAssigneeOptions } from 'configure/reducers/respond/incident-rules/rule/selectors';
import {
...
const cloneRule = (templateRuleId, onSuccess) => {
  return {
    type: ACTION_TYPES.INCIDENT_RULES_CLONE_SAGA,
    templateRuleId,
    onSuccess
  };
};
...
const deleteRule = (ruleId) => {
  return {
    type: ACTION_TYPES.INCIDENT_RULES_DELETE_SAGA,
    ruleId
  };
};
...
const getRules = () => {
  return {
    type: ACTION_TYPES.FETCH_INCIDENT_RULES_SAGA
  };
};
...
const saveRule = (onSuccess) => {
  return (dispatch, getState) => {
    ...
    if (update.id) {
      dispatch({
        type: ACTION_TYPES.SAVE_INCIDENT_RULE_SAGA,
        ruleInfo: update,
        onSuccess
      });
...
const createRule = (ruleInfo, onSuccess) => {
  return {
    type: ACTION_TYPES.CREATE_INCIDENT_RULE_SAGA,
    ruleInfo,
    onSuccess
  };
};
...
const getRule = (ruleId) => {
  return (dispatch) => {
    if (ruleId) {
      dispatch({
        type: ACTION_TYPES.FETCH_INCIDENT_RULE_SAGA,
        ruleId
      });
    } else {
      dispatch(newRule());
...
const getFields = () => {
  return {
    type: ACTION_TYPES.FETCH_INCIDENT_FIELDS_SAGA
  };
};
 ...
 * @public
 */
const newRule = () => ({ type: ACTION_TYPES.NEW_INCIDENT_RULE });
...
const addGroup = () => {
  return {
    type: ACTION_TYPES.INCIDENT_RULES_ADD_GROUP
  };
};
...
const removeGroup = (groupId) => {
  return {
    type: ACTION_TYPES.INCIDENT_RULES_REMOVE_GROUP,
    payload: groupId
  };
};
...
const reorderRules = (ruleIds) => {
  return {
    type: ACTION_TYPES.INCIDENT_RULES_REORDER_SAGA,
    ruleIds
  };
};
