...
public class SmartDataStoreMongoImpl implements SmartDataStore {
    ...
    public List<EntityEvent> readSmarts(TimeRange timeRange, int scoreThreshold) {
        ...
        Criteria startDateTimeCriteria = Criteria.where(EntityEvent.ENTITY_EVENT_START_TIME_UNIX_FIELD_NAME).gte(start);
        Criteria endDateTimeCriteria = Criteria.where(EntityEvent.ENTITY_EVENT_END_TIME_UNIX_FIELD_NAME).lt(end);
