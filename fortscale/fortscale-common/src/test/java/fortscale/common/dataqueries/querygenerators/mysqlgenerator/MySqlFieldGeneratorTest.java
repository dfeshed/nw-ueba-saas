...
public class MySqlFieldGeneratorTest extends DataQueryGeneratorTest {
    ...
    public void setUp()
        ...
        when(dataEntitiesConfig.getAllEntityFields(joinDTO.getEntities()[0])).thenReturn(fieldIds);
        when(dataEntitiesConfig.getEntityTable(joinDTO.getEntities()[0])).thenReturn(entityTable);
		when(dataEntitiesConfig.getFieldTable(eq(joinDTO.getEntities()[0]),any(String.class))).thenReturn(entityTable+ ".");
        when(dataEntitiesConfig.getFieldColumn(entityId, "source_machine")).thenReturn("source_machine_column");
        when(dataEntitiesConfig.getFieldColumn(entityId, "event_time")).thenReturn("event_time_column");
    }
