...
public class GaussianModelScorerAlgorithmTest {
	...
	public void shouldFailWhenGivenNullAsPriorModel() {
		defaultAlgorithm.calculate(null, new GaussianPriorModel(), 0);
	}
    ...
    public void shouldScore0WhenLessThanTwoSamples() {
        IntStream.of(0, 1)
                ...
                .mapToDouble(model -> defaultAlgorithm.calculate(model, null, 1))
                .forEach(score -> Assert.assertEquals(0, score, 0.0000001));
        Assert.assertTrue(defaultAlgorithm.calculate(new ContinuousDataModel().setParameters(2, 0, 0, 0), null, 1) > 0);
    }

    @Test
    public void shouldScore100WhenSdIsZero() {
        Assert.assertEquals(100, defaultAlgorithm.calculate(new ContinuousDataModel().setParameters(10, 0, 0, 0), null, 1), 0.0000001);
    }
    ...
    public void shouldScoreZeroWhenGivenValueLessThanOrEqualToTheMean() {
        ...
        IntStream.range(0, (int) mean)
                .mapToDouble(value -> defaultAlgorithm.calculate(model, null, value))
                .forEach(score -> Assert.assertEquals(0, score, 0.0000001));
    }
    ...
    public void shouldScoreAccordingTo65__95__99dot7_ruleWhenNIsReallyBig() {
        ...
        int N = 100000000;
		double mean = 4.3;
		Assert.assertEquals(68.27, defaultAlgorithm.calculate(new ContinuousDataModel().setParameters(N, mean, sd, 0), null, mean + 1 * sd), 0.01);
        Assert.assertEquals(95.45, defaultAlgorithm.calculate(new ContinuousDataModel().setParameters(N, mean, sd, 0), null, mean + 2 * sd), 0.01);
        Assert.assertEquals(99.73, defaultAlgorithm.calculate(new ContinuousDataModel().setParameters(N, mean, sd, 0), null, mean + 3 * sd), 0.01);
    }
	...
	public void shouldScoreAlmostAccordingTo65__95__99dot7_ruleWhenNIsSmall() {
		...
		int N = 10;
		double mean = 4.3;
		Assert.assertEquals(65.65, defaultAlgorithm.calculate(new ContinuousDataModel().setParameters(N, mean, sd, 0), null, mean + 1 * sd), 0.01);
		Assert.assertEquals(92.34, defaultAlgorithm.calculate(new ContinuousDataModel().setParameters(N, mean, sd, 0), null, mean + 2 * sd), 0.01);
		Assert.assertEquals(98.5, defaultAlgorithm.calculate(new ContinuousDataModel().setParameters(N, mean, sd, 0), null, mean + 3 * sd), 0.01);
	}
	...
	public void shouldScoreDecreasinglyAsPriorIncreases() {
		...
		List<Double> scores = IntStream.range(0, 100)
				.mapToDouble(prior -> defaultAlgorithm.calculate(
						model,
	...
	public void shouldScoreDecreasinglyAsGlobalInfluenceIncreases() {
		...
		List<Double> scores = IntStream.range(0, 100)
				.mapToDouble(globalInfluence -> new GaussianModelScorerAlgorithm(globalInfluence).calculate(
						model,
						new GaussianPriorModel()
	...
	public void shouldIgnorePriorWhenNull() {
		...
		double scoreWithNoPrior = defaultAlgorithm.calculate(model, null, value);
		double scoreWithPriorResultsInNull = defaultAlgorithm.calculate(model, new GaussianPriorModel(), value);
	...
	public void shouldIgnorePriorWhenLessThenSdOfModel() {
		...
		double scoreWithNoPrior = defaultAlgorithm.calculate(model, null, value);
		double scoreWithPriorLessThanSd = defaultAlgorithm.calculate(model, priorModel, value);
	...
	public void shouldResortToMinimalPriorOverOrganizationWhenNoPriorForGivenModel() {
		...
		double scoreForModelWithSmallPrior = defaultAlgorithm.calculate(model, new GaussianPriorModel().init(Arrays.asList(
				new GaussianPriorModel.SegmentPrior(meanOfModel, priorSmall, 0),
				new GaussianPriorModel.SegmentPrior(otherMean1, priorBig, 0)
		)), value);
		double scoreForModelWithNoPrior = defaultAlgorithm.calculate(model, new GaussianPriorModel().init(Arrays.asList(
