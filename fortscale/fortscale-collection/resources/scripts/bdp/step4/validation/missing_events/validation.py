...
def _validate(host, start, end):
    ...
    entity_events_counter = get_collections_size(host=host, collection_names_regex='^entity_event_(?!meta_data_).', find_query={'endTime': time_interval})
    scored_entity_events_counter = get_collections_size(host=host, collection_names_regex='^scored___entity_event_', find_query={'end_time_unix': time_interval})
