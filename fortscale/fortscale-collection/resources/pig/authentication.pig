...
userWithComp    = FOREACH notSameHost GENERATE generatedTime,LOWER(account_name) as account_name,LOWER(normalized_dst_machine),(LOWER(is_nat)=='true'? '': (normalized_src_machine is null?'':LOWER(normalized_src_machine))),((failure_code is null) or ((failure_code != '0x12') and (failure_code != '0x22')) ? '0x0' : generatedTime),failure_code,normalized_username,eventCode,(computer_name is null?client_address:(computer_name==''?client_address:LOWER(computer_name))),src_class,dst_class,LOWER(service_name);
