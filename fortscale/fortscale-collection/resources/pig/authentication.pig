...
userWithCompOrdered = FOREACH onlyUsersNoDC GENERATE generatedTime,LOWER(account_name) as account_name,LOWER(service_name),(computer_name is null?client_address:(computer_name==''?client_address:LOWER(computer_name))),failure_code,normalized_username;
user            = GROUP userWithCompOrdered by normalized_username PARALLEL 1;
