...
loginFields     = FOREACH loginUsersOnlyNoFailedDC GENERATE generatedTime,LOWER(accountName) as account_name,LOWER(machineName) as service_name,LOWER(machineName) as source_machine,((failureCode is null) or ((failureCode != '0x12') and (failureCode != '0x22')) ? '0x0' : generatedTime),failureCode,normalized_username;
