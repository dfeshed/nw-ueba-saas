...
selectedFields = FOREACH loginByTime GENERATE date_time,LOWER(user_name) as user_name,LOWER(normalized_dst_machine) as score_target_machine,(normalized_src_machine is null? ip : (normalized_src_machine=='' ? ip : LOWER(normalized_src_machine))) as normalized_src_machine,LOWER(status) as status,LOWER(auth_method) as auth_method,normalized_username,target_machine,(client_hostname is null? ip : (client_hostname=='' ? ip : LOWER(client_hostname))) as client_hostname;
