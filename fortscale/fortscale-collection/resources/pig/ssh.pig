...
selectedFields = FOREACH loginByTime GENERATE date_time,LOWER(user_name) as user_name,LOWER(target_machine) as target_machine,(client_hostname is null? ip : (client_hostname=='' ? ip : LOWER(client_hostname))),LOWER(status) as status,LOWER(auth_method) as auth_method;
