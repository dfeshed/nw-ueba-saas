...
public class AggrFeatureEventBuilderTest {
    ...
    private void assertEvent(JSONObject event, int startTimeDayNumber, int endTimeDayNumber, Long numberOfDistinctValues) {
        ...
        Assert.assertEquals("aggregated_feature_event", event.getAsString("event_type"));
        Assert.assertEquals("bc1", event.getAsString("bucket_conf_name"));
        ...
        Assert.assertEquals(endTime, event.getAsNumber("end_time_unix"));
