...
public class ReductingScorerTest extends ScorerBaseTest{
	...
	protected Scorer buildScorer(String scorerName, String outputFieldName, String mainScorer, String reductingScorer, Double reducting){
		return buildScorer(scorerName, outputFieldName, mainScorer, reductingScorer, reducting, null);
	}
	
	@SuppressWarnings("unchecked")
	protected Scorer buildScorer(String scorerName, String outputFieldName, String mainScorer, String reductingScorer, Double reducting, Double reductingZeroScoreWeight){
		if(scorerName !=null){
			...
			if(reductingZeroScoreWeight != null)
				when(config.getDouble(String.format("fortscale.score.%s.reducting.zero.score.weight", scorerName))).thenReturn(reductingZeroScoreWeight);
			else
				when(config.getDouble(String.format("fortscale.score.%s.reducting.zero.score.weight", scorerName),ReductingScorer.REDUCTING_ZERO_SCORE_WEIGHT_DEFAULT)).thenReturn(ReductingScorer.REDUCTING_ZERO_SCORE_WEIGHT_DEFAULT);
	...
	private void testScore(Scorer scorer, EventMessage eventMessage, int mainScore, int reductingScore, double reducting) throws Exception{
		testScore(scorer, eventMessage, mainScore, reductingScore, reducting, ReductingScorer.REDUCTING_ZERO_SCORE_WEIGHT_DEFAULT);
	}
	
	private void testScore(Scorer scorer, EventMessage eventMessage, int mainScore, int reductingScore, double reducting, double reductingZeroScoreWeight) throws Exception{
