...
public class EsperRulesTest {
    ...
    public void testSmartEventWithNotificationTest() throws Exception{
        ...
        String createTimestamp = "create variable Long currentTimestamp ="+(currentTimeStamp+(60*60*1000+31*60*1000)); // time now + 1 hour and 31 min
        String createLastEventTimestamp = "create variable Long lastEventTimestamp =" + (currentTimeStamp +(60*31*1000)); // bigger in half hour since the current timestamp
        ...
        //
        // String esperRule = "select Notification.entityName as entityName from EnrichedEvidence(evidenceType = EvidenceType.Notification).win:expr_batch(oldest_timestamp+(60*60*1000+30*60*1000) < currentTimestamp or (oldest_event.hourlyStartDate is not null and lastEventTimestamp > 30*60*1000+hourEndTimestamp(oldest_event.hourlyStartDate))).std:size(entityType,entityName) as Notification";

        String esperRule = "select  entityName,size  from EnrichedEvidence(evidenceType = EvidenceType.Notification).win:expr_batch(oldest_timestamp+(60*60*1000+30*60*1000) < currentTimestamp or (oldest_event.hourlyStartDate is not null and lastEventTimestamp > 30*60*1000+hourEndTimestamp(oldest_event.hourlyStartDate))).std:size(entityType,entityName) as Notification";
        ...
        EPStatement stmt = epService.getEPAdministrator().createEPL(esperRule);
        ...
        notification = new Evidence(EntityType.User, "entityTypeFieldName", "user1@fs.com", EvidenceType.Notification,eventStartData ,eventStartData +1 ,"anomalyTypeFieldName","anomalyValue",new ArrayList<String>(),99,Severity.Critical,3,EvidenceTimeframe.Hourly);
        epService.getEPRuntime().sendEvent(notification);
