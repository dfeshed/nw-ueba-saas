...
public class UnifiedAlertIntegrationTestHelper {
    ...
    public void assertCreateIndicatorListApplicableForDecider(List<EnrichedFortscaleEvent> expectedToSendToMethod, List<EnrichedFortscaleEvent> expectedToReturnFromMethod) {
        ...
        Mockito.verify(evidencesApplicableToAlertService, Mockito.atLeast(1)).createIndicatorListApplicableForDecider(
                applicableCandidatesCaptor.capture(), Mockito.anyLong(), Mockito.anyLong(), Mockito.any());
        ...
        List<EnrichedFortscaleEvent> newExpected = evidencesApplicableToAlertService.createIndicatorListApplicableForDecider
                (expectedToSendToMethod, 0L, 0L, AlertTimeframe.Daily);
     ...
     */
    public void assertScoreDecider(List<EnrichedFortscaleEvent> expectedToSendToMethod, String expectedAlertName, AlertTimeframe alertTimeframe) {
        //Test name decider executed
        ArgumentCaptor<List> enrichedFortscaleEventsToNameDecider = ArgumentCaptor.forClass(List.class);
        ArgumentCaptor<AlertTimeframe> alertTimeFrameCatcher = ArgumentCaptor.forClass(AlertTimeframe.class);

        Mockito.verify(decider, Mockito.atLeast(1)).decideName(enrichedFortscaleEventsToNameDecider.capture(), alertTimeFrameCatcher.capture());
        ...
        String title = decider.decideName(expectedToSendToMethod, alertTimeframe);
     ...
     * @param expectedScore
     */
    public void assertScoreDecider(List<EnrichedFortscaleEvent> expectedToSendToMethod, int expectedScore, AlertTimeframe alertTimeFrame) {
        //Test score decider executed
        ArgumentCaptor<List> enrichedFortscaleEventsToNameDecider = ArgumentCaptor.forClass(List.class);
        Mockito.verify(decider, Mockito.atLeastOnce()).decideScore(enrichedFortscaleEventsToNameDecider.capture(), Mockito.any());
        ...
        int score = decider.decideScore(expectedToSendToMethod, alertTimeFrame);
     ...
     * This method test that saveAlertInRepository has been called with expected alert
     */
    public void assertAlertWasntCreated() {
        resetMocks();
        ArgumentCaptor<Alert> alertCaptor = ArgumentCaptor.forClass(Alert.class);
        Mockito.verify(alertsService, Mockito.atMost(0)).saveAlertInRepository(alertCaptor.capture());
    }

    /**
     * This method cleans the data from all the mocks we use - critical for method called validations
     */
    public void resetMocks() {
        Mockito.reset(alertsService);
        Mockito.reset(decider);
        Mockito.reset(evidencesApplicableToAlertService);
    }
}
