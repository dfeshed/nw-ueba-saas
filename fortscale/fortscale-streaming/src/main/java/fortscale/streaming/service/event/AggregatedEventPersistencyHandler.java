...
public class AggregatedEventPersistencyHandler implements EventPersistencyHandler, InitializingBean {
    ...
    public void saveEvent(JSONObject event, String collectionPrefix) {
        ...
        if (!isCollectionExist(collectionName)) {
            ...
            mongoTemplate.indexOps(collectionName).ensureIndex(new FIndex().expire(retentionTimeInSeconds, TimeUnit.SECONDS).named(AggrEvent.EVENT_FIELD_END_TIME).on(AggrEvent.EVENT_FIELD_END_TIME, Sort.Direction.DESC));
