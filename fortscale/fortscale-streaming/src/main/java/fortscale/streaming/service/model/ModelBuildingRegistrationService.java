...
public class ModelBuildingRegistrationService {
	...
	public void process(JSONObject event) {
		...
		boolean selectHighScoreContexts = modelBuildingCommandMessage.isSelectHighScoreContexts();
		Set<String> specifiedContextIds = modelBuildingCommandMessage.getSpecifiedContextIds();
			...
			while (iterator.hasNext()) {
				...
				if (reg == null) {
					...
					modelService.process(modelBuildingListener, reg.getSessionId(), reg.getModelConfName(),
							reg.getPreviousEndTime(), reg.getCurrentEndTime(),reg.selectHighScoreContexts(),reg.getSpecifiedContextIds());
