...
public class UserRepositoryImpl implements UserRepositoryCustom {
	...
	public User findByApplicationUserName(
		...
		String appUserNameField = User
				.getAppUserNameField(applicationUserDetails
						.getApplicationName());
		Query query = new Query(where(appUserNameField).is(
	...
	public User findByObjectGUID(String objectGUID) {
		Query query = new Query(where(
				User.getAdInfoField(UserAdInfo.objectGUIDField)).is(objectGUID));
		return mongoTemplate.findOne(query, User.class);
	}
	...
	public List<User> findByApplicationUserName(String applicationName,
		...
		for (int i = 0; i < usernames.size(); i++) {
			...
			criterias[i] = where(appUserNameField).regex(
					String.format("^%s$", username), "i");
		}
	...
	public User findByApplicationUserName(String applicationName,
			String username) {
		String appUserNameField = User.getAppUserNameField(applicationName);
		Query query = new Query(where(appUserNameField).regex(
				String.format("^%s$", username), "i"));
		return mongoTemplate.findOne(query, User.class);
	}
	...
	@Override
	public Page<User> findByClassifierIdAndScoreBetweenAndTimeGteAsData(
			String classifierId, int lowestVal, int upperVal, Date time,
			Pageable pageable) {
		String classifierScoreCurrentTimestampField = User
				.getClassifierScoreCurrentTimestampField(classifierId);
		String classifierCurScoreField = User
				.getClassifierScoreCurrentScoreField(classifierId);
		Query query = new Query(where(classifierCurScoreField).gte(lowestVal)
				.lte(upperVal).and(classifierScoreCurrentTimestampField)
				.gte(time));
	...
	@Override
	public Page<User> findByClassifierIdAndFollowedAndScoreBetweenAndTimeGteAsData(
			String classifierId, int lowestVal, int upperVal, Date time,
			Pageable pageable) {
		String classifierScoreCurrentTimestampField = User
				.getClassifierScoreCurrentTimestampField(classifierId);
		String classifierCurScoreField = User
				.getClassifierScoreCurrentScoreField(classifierId);
		Query query = new Query(where(User.followedField).is(true)
				.and(classifierCurScoreField).gte(lowestVal).lte(upperVal)
				.and(classifierScoreCurrentTimestampField).gte(time));

		return getPage(query, pageable, User.class);
	}

	private <T> Page<T> getPage(Query query, Pageable pageable,
			Class<T> entityClass) {
		query.with(pageable);
	...
	@Override
	public Page<User> findByClassifierIdAndTimeGteAsData(String classifierId,
			Date time, Pageable pageable) {
		String classifierScoreCurrentTimestampField = User
				.getClassifierScoreCurrentTimestampField(classifierId);
		Query query = new Query(where(classifierScoreCurrentTimestampField)
				.gte(time));
	...
	@Override
	public Page<User> findByClassifierIdAndFollowedAndTimeGteAsData(
			String classifierId, Date time, Pageable pageable) {
		String classifierScoreCurrentTimestampField = User
				.getClassifierScoreCurrentTimestampField(classifierId);
		Query query = new Query(where(User.followedField).is(true)
				.and(classifierScoreCurrentTimestampField).gte(time));
	...
	public int countNumOfUsersAboveThreshold(String classifierId,
		...
		String classifierCurScoreField = User
				.getClassifierScoreCurrentScoreField(classifierId);
		Query query = new Query(where(classifierCurScoreField)
				.gte(threshold.getValue())
				.and(classifierScoreCurrentTimestampField)
				.gte(dateTime.toDate()));
	...
	public void updateFollowed(User user, boolean followed) {
		mongoTemplate.updateFirst(query(where(User.ID_FIELD).is(user.getId())),
				update(User.followedField, followed), User.class);
	}
	...
	public List<User> findByGUIDs(Collection<String> guids) {
		return findByUniqueField(
				User.getAdInfoField(UserAdInfo.objectGUIDField), guids);
	}
	...
	public User findByAdEmailAddress(EmailAddress emailAddress) {
		return findOneByField(
				User.getAdInfoField(UserAdInfo.emailAddressField), emailAddress);
	}

	@Override
	public List<User> findByAdLastnameContaining(String lastNamePrefix) {
		Query query = new Query(where(
				User.getAdInfoField(UserAdInfo.lastnameField)).regex(
				String.format("^%s$", lastNamePrefix), "i"));
		return mongoTemplate.find(query, User.class);
	}
	...
	public User findByAdUserPrincipalName(String adUserPrincipalName) {
		return findOneByField(
				User.getAdInfoField(UserAdInfo.userPrincipalNameField),
				adUserPrincipalName);
	}
	...
	public List<User> findByAdUserPrincipalNameContaining(
		...
		Query query = new Query(where(
				User.getAdInfoField(UserAdInfo.userPrincipalNameField)).regex(
				String.format("^%s$", adUserPrincipalNamePrefix), "i"));
		return mongoTemplate.find(query, User.class);
	}
	...
	public User findByAdInfoObjectGUID(String objectGUID) {
		return findOneByField(User.getAdInfoField(UserAdInfo.objectGUIDField),
				objectGUID);
	}
	...
	public HashMap<String, String> findAllUsernames() {
		...
		for (UsernameWrapper username : mongoTemplate.find(query,
				UsernameWrapper.class, User.collectionName)) {
	...
	public void updateUserServiceAccount(User user, boolean isUserServiceAccount) {
		mongoTemplate.updateFirst(query(where(User.ID_FIELD).is(user.getId())),
				update(User.userServiceAccountField, isUserServiceAccount),
				User.class);
	}

	@Override
	public List<User> findByUserInGroup(Collection<String> groups) {
		return findByUniqueField(User.getAdInfoField(String.format("%s.%s",
				UserAdInfo.groupsField, UserAdInfo.adDnField)), groups);
	}
	...
	public void updateAdministratorAccount(User user,
		...
		mongoTemplate.updateFirst(query(where(User.ID_FIELD).is(user.getId())),
				update(User.administratorAccountField, isAdministratorAccount),
				User.class);
	}

	@Override
	public void updateCurrentUserScore(User user, String classifierId,
			double score, double trendScore, DateTime calculationTime) {
		Update update = new Update();
		update.set(User.getClassifierScoreCurrentScoreField(classifierId),
				score);
		update.set(User.getClassifierScoreCurrentTrendScoreField(classifierId),
				trendScore);
		update.set(User.getClassifierScoreCurrentTimestampField(classifierId),
				calculationTime.toDate());
		update.set(
				User.getClassifierScoreCurrentTimestampEpochField(classifierId),
				calculationTime.getMillis());
