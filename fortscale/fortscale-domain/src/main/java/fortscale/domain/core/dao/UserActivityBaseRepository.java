...
public abstract class UserActivityBaseRepository {
    ...
    protected <T extends UserActivityDocument> List<T> getUserActivityEntries(@Nullable String username, int timeRangeInDays, String collectionName, Class<T> documentType) {
        ...
        Criteria jointCriteria = Criteria.where(UserActivityDocument.START_TIME_FIELD_NAME).gte(TimestampUtils.convertToSeconds(getStartTime(timeRangeInDays)));
        if (username != null) {
            Criteria idCriteria = Criteria.where(UserActivityDocument.ENTITY_ID_FIELD_NAME).is(username);
