...
public class ApiClassifierController extends BaseController {
	...
	public DataBean<List<?>> loginsEvents(@RequestParam(required=false) Long date,
		...
		return events(LogEventsEnum.login, date, uid, offset, limit, orderBy, orderByDirection, minScore, model);
	}
	...
	public DataBean<List<?>> events(@PathVariable LogEventsEnum id, @RequestParam(required = false) Long date, @RequestParam(required = false) String uid, @RequestParam(defaultValue = "0") Integer offset,
			@RequestParam(defaultValue = "10") Integer limit, @RequestParam(required = false) String orderBy, @RequestParam(defaultValue = "DESC") String orderByDirection, @RequestParam(defaultValue = "0") Integer minScore, Model model) {
		Date timestamp = null;
		if (date != null) {
		...
		case ssh:
		case login:
			ret = authEvents(id, timestamp, uid, offset, limit, orderByDirection, direction, minScore);
			break;
		case vpn:
			ret = vpnEvents(id, timestamp, uid, offset, limit, orderByDirection, direction, minScore);
		default:
			break;
		}
	...
	private DataBean<List<?>> authEvents(LogEventsEnum id, Date timestamp, String uid, Integer offset, Integer limit, String orderBy, Direction direction, Integer minScore) {
		DataBean<List<?>> ret = new DataBean<List<?>>();

		List<ILoginEventScoreInfo> eventScoreInfos = Collections.emptyList();
		int total = 0;
		if (uid == null) {
			eventScoreInfos = classifierService.getSuspiciousAuthEvents(id, timestamp, offset, limit, orderBy, direction, minScore);
	...
	private DataBean<List<?>> vpnEvents(LogEventsEnum id, Date timestamp, String uid, Integer offset, Integer limit, String orderBy, Direction direction, Integer minScore) {
		DataBean<List<?>> ret = new DataBean<List<?>>();
