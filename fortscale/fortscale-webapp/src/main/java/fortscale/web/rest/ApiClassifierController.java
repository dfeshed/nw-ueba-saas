...
public class ApiClassifierController {
	...
	@ResponseBody
	@LogException
	public DataBean<List<IClassifierScoreDistribution>> dist(@PathVariable String id, Model model){
		DataBean<List<IClassifierScoreDistribution>> ret = new DataBean<List<IClassifierScoreDistribution>>();
		List<IClassifierScoreDistribution> dists = classifierService.getScoreDistribution(id);
		ret.setData(dists);
