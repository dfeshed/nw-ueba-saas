...
public class IseResolverTest {
	...
	private IseEvent createIseEvent(String ip, String hostname, long timestampepoch, long expiration) {
	...
	public void addIseEvent_should_replace_the_cache_if_the_item_in_cache_is_older_than_given_event() {
		...
		// act
		iseResolver.addIseEvent(createIseEvent("192.168.1.1", "or-me", now+300, now+400));
		
		// verify
		verify(cache).put("192.168.1.1", createIseEvent("192.168.1.1", "or-me", now+300, now+400));
	...
	public void addIseEvent_should_not_update_the_cache_if_the_item_in_cache_is_newer_than_given_event() {
		// mock old cache value
		when(cache.get("192.168.1.1")).thenReturn(createIseEvent("192.168.1.1", "pick-me", now+300, now+400));
		
		// act
		iseResolver.addIseEvent(createIseEvent("192.168.1.1", "or-me", now+100, now+200));
	...
	public void getLatestIseEventBeforeTimestamp_should_return_ise_event_from_cache_if_it_is_not_expired_before_given_timestamp() {
		IseEvent cached = createIseEvent("192.168.1.1", "pick-me", now+100, now+200);
	...
	public void getLatestIseEventBeforeTimestamp_should_return_event_from_repository_if_cached_event_is_expired_at_given_timestamp() {
		...
		IseEvent saved = createIseEvent("192.168.1.1", "not", now+210, now+500);
	...
	public void removeFromBlackList_should_remove_from_blacklist_when_new_time_range_intersact_with_the_old_one(){
		IseEvent newEvent = createIseEvent("192.168.1.1", "pick-me", now+100, now+200);
