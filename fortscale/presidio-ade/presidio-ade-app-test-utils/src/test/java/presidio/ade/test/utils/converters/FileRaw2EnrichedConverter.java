...
public class FileRaw2EnrichedConverter implements Converter<FileEvent, EnrichedFileRecord> {
    ...
    public EnrichedFileRecord convert(FileEvent source) {
        ...
        enrichedEvent.setOperationType(source.getFileOperation().getOperationType().getName());
        enrichedEvent.setOperationTypeCategories(source.getFileOperation().getOperationType().getCategories());
