...
moduleForComponent('host-list/host-table/pivot-to-investigate', 'Integration | Component | Pivot to investigate', {
  integration: true,
...
test('it renders', function(assert) {
  this.render(hbs`{{host-list/host-table/pivot-to-investigate}}`);
...
test('should render the service list', function(assert) {
  ...
  this.render(hbs`
    {{host-list/host-table/pivot-to-investigate serviceList=serviceList}}`);
  this.$('.rsa-icon').trigger('click');
  return wait().then(() => {
    assert.equal($('#modalDestination .service-modal').length, 1, 'Expected to render service modal');
...
test('should enable the navigate button on selecting the service', function(assert) {
  ...
  this.render(hbs`{{host-list/host-table/pivot-to-investigate metaName='machine.machineName' serviceList=serviceList}}`);
  this.$('.rsa-icon').trigger('click');
  return wait().then(() => {
    assert.equal($('#modalDestination .service-modal').length, 1, 'Expected to render service modal');
    assert.equal($('#modalDestination .rsa-data-table').length, 1, 'Expected to render rsa data table');
...
test('should open the investigate page in new window', function(assert) {
  ...
  this.render(hbs`{{host-list/host-table/pivot-to-investigate metaName='machine.machineName' item=item serviceList=serviceList}}`);
  this.$('.rsa-icon').trigger('click');
  return wait().then(() => {
    assert.equal($('#modalDestination .service-modal').length, 1, 'Expected to render service modal');
    assert.equal($('#modalDestination .rsa-data-table').length, 1, 'Expected to render rsa data table');
