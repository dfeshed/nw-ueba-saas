...
module('Integration | Component | property-panel-policy/edr-policy', function(hooks) {
  ...
  test('Content data', async function(assert) {
    new ReduxDataHelper(setState).policy(policyData).build();
    await render(hbs`{{property-panel-policy/edr-policy}}`);
    assert.equal(document.querySelectorAll('.edr-value').length, 17, 'All values are showing');
    ...
    assert.equal(document.querySelectorAll('.content-section__section-name')[3].textContent.trim(), 'Download Settings', 'Download settings section shows');
    assert.equal(document.querySelectorAll('.content-section__section-name')[4].textContent.trim(), 'Response Action Settings', 'Response Action Settings section shows');
    assert.equal(document.querySelectorAll('.content-section__section-name')[5].textContent.trim(), 'Endpoint Server Settings', 'Endpoint Server Settings section shows');
  ...
  test('Content data scheduledScanConfigDisabled', async function(assert) {
    ...
    await render(hbs`{{property-panel-policy/edr-policy}}`);
    assert.equal(document.querySelectorAll('.edr-value').length, 12, 'All values are showing minus five fields in Scan Schedule');
