...
module('Integration | Component | Anomalies/Hooks', function(hooks) {
  ...
  test('Image Hooks column names', async function(assert) {
    ...
    assert.equal(find('.rsa-data-table-header-row .rsa-data-table-header-cell:nth-child(3)').textContent.trim(), 'Hooked Process', 'Column 3 is Hooked Process');
    ...
    assert.equal(find('.rsa-data-table-header-row .rsa-data-table-header-cell:nth-child(5)').textContent.trim(), 'Hooked Symbol', 'Column 5 is Hooked Symbol');
  });
});