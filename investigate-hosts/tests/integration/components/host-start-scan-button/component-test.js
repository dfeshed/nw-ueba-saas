...
moduleForComponent('host-start-scan-button', 'Integration | Component | Host Scan Start Button', {
  integration: true,
...
test('it renders the scan button', function(assert) {
  this.render(hbs`{{host-start-scan-button}}`);
...
test('it renders the scan button with text', function(assert) {
  ...
  this.render(hbs`{{host-start-scan-button buttonText=buttonText}}`);
...
test('it renders the only icon', function(assert) {
  ...
  this.render(hbs`{{host-start-scan-button isIconOnly=isIconOnly}}`);
...
test('it should show scan start modal on clicking the button', function(assert) {
  ...
  this.render(hbs`{{host-start-scan-button modalTitle=modalTitle}}`);
...
test('it should show warning messages', function(assert) {
  ...
  this.render(hbs`{{host-start-scan-button warningMessage=warningMessage}}`);
  this.$('.rsa-form-button').trigger('click');
  return wait().then(() => {
...
test('it should show success message start scan is success', function(assert) {
  ...
  this.set('agentIds', [1]);
  this.render(hbs`{{host-start-scan-button agentIds=agentIds}}`);
