...
public class AuthenticationOperationTypeCyclicGenerator extends CyclicValuesGenerator<OperationType> implements IOperationTypeGenerator {
    ...
    private static OperationType[] getDefaultAuthenticationOperationTypes(){
        ...
        ret.add(new OperationType(AUTHENTICATION_OPERATION_TYPE.INTERACTIVE.value, Arrays.asList(new String[] {"INTERACTIVE"})));
        ret.add(new OperationType(AUTHENTICATION_OPERATION_TYPE.REMOTE_INTERACTIVE.value, Arrays.asList(new String[] {"REMOTE_INTERACTIVE"})));
