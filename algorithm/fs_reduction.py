    ...
    for user_fs in f.iter_fs_by_users():
        ...
        for a in sorted(user_fs, key = lambda a: a['start_time_unix']):
            ...
            if len(user_history) > 0 and weight > 0:
                if abs(1. * sum(user_history) / len(user_history) - a['value']) <= max_bad_value_diff:
                    hist = false_positives_values_hist
                else:
                    hist = true_positives_values_hist
                hist[a['value']] = hist.get(a['value'], 0) + weight
...
def calc_min_value_for_not_reduce(f, score_to_weight):
    hists = find_positive_values_hists(f, max_bad_value_diff = 2, score_to_weight = score_to_weight)
