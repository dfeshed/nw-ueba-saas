...
const schemaFilter = (indicatorSchema) => {
  const SCHEMA_QUERY = {
...
const navigateToInvestigateEventsAnalysis = (entityType, entityValue, indicatorSchema, eventTime, serviceId, additionalFilter) => {
  ...
  const queryParams = {
    ...
    mf: encodeURIComponent(`${schemaFilter(indicatorSchema)} && (${entityFilter(entityType, entityValue)})${additionalFilter}${eventTimeWindow}`), // Meta filter
...
export const navigateToInvestigate = (entityType, entityValue, indicatorSchema, eventTime, item, { linkField, field, additionalFilter }, brokerId) => {
  if (processOrLinkFields.includes(linkField)) {
    ...
    const extraQueryParam = additionalFilter ? ` && ${additionalFilter.replaceAll('${objValue}', `'${encodeURIComponent(item[field])}'`)}` : '';
