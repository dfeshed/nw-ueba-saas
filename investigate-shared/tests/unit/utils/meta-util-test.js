...
test('transform returns fields from map or specific object', function(assert) {
  ...
  assert.equal(keys[10], 'agent_id', 'agent.id => agent_id');
  assert.equal(keys[11], 'hostname', 'alias.host => hostname');
  assert.equal(keys[12], 'category', 'category => category');
  assert.equal(keys[13], 'type', 'type => type');
  assert.equal(keys[14], 'domain', 'domain => domain');
