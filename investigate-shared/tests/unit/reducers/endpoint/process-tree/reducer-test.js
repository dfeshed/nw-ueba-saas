...
import reducer from 'investigate-shared/reducers/endpoint/process-tree/reducer';
...
module('Unit | Reducers | process-tree', function() {
  ...
  test('SET_PROCESS_ANALYSIS_INPUT query input to state', function(assert) {
    ...
    const result = reducer(previous, { type: ACTION_TYPES.SET_PROCESS_ANALYSIS_INPUT, payload: { processName: 'test', agentId: 1 } });
    assert.equal(result.queryInput.processName, 'test');
  });
