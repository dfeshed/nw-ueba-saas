...
module('Integration | Component | endpoint/process-analysis/process-tree', function(hooks) {
  ...
  test('raghs it renders the process tree', async function(assert) {
    const treeData = {
    ...
    await render(hbs`{{endpoint/process-analysis/process-tree treeData=treeData}}`);
  ...
  test('it should expand the node on click', async function(assert) {
    ...
    assert.equal(findAll('.process').length, 5, 'Expected to render 7 nodes');
  });
});
