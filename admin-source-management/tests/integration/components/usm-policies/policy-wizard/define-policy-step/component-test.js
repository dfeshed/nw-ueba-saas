...
module('Integration | Component | usm-policies/policy-wizard/define-policy-step', function(hooks) {
  ...
  test('All the components in the available settings is rendered on the UI', async function(assert) {
    ...
    await render(hbs`{{usm-policies/policy-wizard/define-policy-step}}`);
    assert.equal(findAll('.available-settings .available-setting').length, 20, 'All available settings rendered on the UI');
  });
  ...
  test('When policy is Windows Log policy, All the components in the available settings is rendered on the UI', async function(assert) {
    ...
    await render(hbs`{{usm-policies/policy-wizard/define-policy-step}}`);
    assert.equal(findAll('.available-settings .available-setting').length, 7, 'All windows log policy available settings rendered on the UI');
  });
  ...
  test('For the Windows Log Policy, Advanced setting is rendered on the UI', async function(assert) {
    new ReduxDataHelper(setState).policyWiz('windowsLogPolicy').build();
    await render(hbs`{{usm-policies/policy-wizard/define-policy-step}}`);
  ...
  test('Labels, sub-headers and components rendered correctly in available settings', async function(assert) {
    new ReduxDataHelper(setState).policyWiz().build();
    await render(hbs`{{usm-policies/policy-wizard/define-policy-step}}`);
    assert.equal(findAll('.available-settings .heading').length, 7, 'All heading labels rendered correctly');
    assert.equal(findAll('.available-settings .title').length, 20, 'All components in available-settings rendered correctly');
    assert.equal(findAll('.available-settings .rsa-icon-add-circle-1').length, 20, 'The plus icon next to the components is rendered correctly');
  });
