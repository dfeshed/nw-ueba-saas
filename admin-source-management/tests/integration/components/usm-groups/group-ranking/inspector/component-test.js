...
module('Integration | Component | group-ranking/inspector | Policy Inspector', function(hooks) {
  ...
  test('It shows the common sections for history and groups', async function(assert) {
    ...
    await render(hbs`{{usm-groups/group-ranking/inspector}}`);
    assert.equal(findAll('.usm-ranking-inspector .heading').length, 5, 'expected headings are shown');
    assert.equal(findAll('.usm-ranking-inspector .title').length, 13, 'expected property names are shown');
    assert.equal(findAll('.usm-ranking-inspector .value').length, 36, 'expected value elements are shown');
  });
});