...
import policiesData from '../../../data/subscriptions/policy/findAll/data';
...
module('Unit | Reducers | Group Reducers', function() {
  ...
  test('on SAVE_GROUP start, groupStatus is properly set', function(assert) {
    ...
    const endState = reducers(Immutable.from(initialState), action);
    assert.deepEqual(endState, expectedEndState, 'groupStatus is wait');
  });
  ...
  test('on SAVE_GROUP success, group & groupStatus are properly set', function(assert) {
    ...
    const endState = reducers(Immutable.from(initialState), action);
    assert.deepEqual(endState, expectedEndState, 'group populated & groupStatus is complete');
  });
