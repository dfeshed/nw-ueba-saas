 ...
 *     <li>The
 *     {@link #setMessageConverters(HttpMessageConverter[]) HttpMessageConverters}
 *     are consulted (in iteration order) with this object result for rendering.  The first
 *     {@code HttpMessageConverter} instance that {@link HttpMessageConverter#canWrite(Class, MediaType) canWrite}
 ...
 * want to match the JSON representation but do not want to use Jackson
 * (or the Spring's default Jackson config), you will need to
 * {@link #setMessageConverters(HttpMessageConverter[]) configure} a different
...
public class RestExceptionHandler extends AbstractHandlerExceptionResolver implements InitializingBean {
    ...
    protected ModelAndView doResolveException(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) {
        ...
        try {
            ...
            logger.error("Acquiring ModelAndView for Exception [{}] resulted in an exception.", invocationEx);
        }
