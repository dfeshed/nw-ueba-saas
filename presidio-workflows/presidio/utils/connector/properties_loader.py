...
def load_and_get_property(property_to_get, filepath, sep='=', comment_char='#'):
    ...
    """
    # java-props escapes char ':' and python just reads it as string
    property_to_get = property_to_get.replace(":", "\\:")
    try:
        value = load_properties(filepath, sep, comment_char)[property_to_get]
    except Exception:
        return None
    return value
