...
class InputPreProcessingDagBuilder(PresidioDagBuilder):
    ...
    def build(self, dag):
        ...
        for pre_processor in pre_processors:
            ...
            type = pre_processor.get(PRE_PROCESSORS_TYPE_KEY)
            static_arguments = pre_processor.get(STATIC_ARGUMENTS_KEY)
            dynamic_arguments = pre_processor.get(DYNAMIC_ARGUMENTS_KEY)
            self._add_input_pre_processor_operator(dag, name, type, schema_name, static_arguments, dynamic_arguments)
        return dag

    def _add_input_pre_processor_operator(self, dag, name, type, schema_name, static_arguments, dynamic_arguments):
        InputPreProcessorOperator(dag=dag, name=name, type=type, schema_name=schema_name,
