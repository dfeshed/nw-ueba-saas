...
class AdapterOperator(FixedDurationJarOperator):
    """
    Runs an adapter task (a JAR file) using a bash command.
    ...
    def __init__(self, fixed_duration_strategy, command, schema, task_id=None, *args, **kwargs):
        ...
        :type task_id: string
        """
        self.log.debug('adapter operator init kwargs=%s', str(kwargs))

        self.fixed_duration_strategy = fixed_duration_strategy
        self.schema = schema
        self.task_id = task_id or 'adapter_{}'.format(schema)
        ...
        super(AdapterOperator, self).__init__(
