...
import { StreamCache, Socket } from './data-access';
...
export default Service.extend({
   ...
   * @private
   */
  _baseAsserts(method, modelName, query, name) {
   ...
   * @public
   */
  streamRequest({ method, modelName, query, onResponse,
      streamOptions = {}, onInit, onStopped, onCompleted, onError
    }) {

    this._baseAsserts(method, modelName, query, 'streamRequest');
    ...
    StreamCache.registerStream(stream, this.get('router').get('currentRouteName'), streamOptions);
   ...
   * @public
   */
  promiseRequest({ method, modelName, query, onInit, streamOptions = {} }) {
    let stream;
    this._baseAsserts(method, modelName, query, 'promiseRequest');
